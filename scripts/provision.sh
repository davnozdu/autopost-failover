...
# ВАЖНО про LLM: штатно ключ основного канала лежит в конфиге (БД) и приезжает с
# бэкапом. Но если дома он задан ПЕРЕМЕННОЙ ОКРУЖЕНИЯ (а поля в настройках пустые),
# в бэкапе его нет — и резерв остался бы без основного канала, свалившись на
# резервные. Поэтому пробрасываем LLM_* и из секретов, как страховку: пустые
# значения ничего не ломают (в конфиге приоритет у БД).
env_file=$(cat <<ENVEOF
DATA_DIR=data
TZ=${TZ:-Europe/Prague}

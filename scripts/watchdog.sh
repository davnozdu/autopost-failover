...
#
# Этап 3 (сейчас): только детект + уведомление + вывод состояния.
# Этап 4/5 (дальше): по состоянию поднимать/гасить резервный сервер на Hetzner.
#
...
if [ "$state" = "HOME_UP" ]; then
  ...
  notify "🔴 autopost: домашний сервер недоступен (проверено ${RETRIES}×$((RETRY_INTERVAL/60))мин). Этап 4 поднимет резерв на Hetzner."
fi

...
# ── 2а. Дать резерву договорить: дождаться деплоя ленты и дослать анонсы ──
# Новость уходит в Telegram только после того, как деплой публичного репо 24n
# положил её на сайт. Деплой идёт до ~12 минут, и резерв, погашенный раньше,
# оставляет последнюю партию без анонсов (2026-07-27: 9 статей, среди них первая
# в разделе «Досуг», деплой закончился через 7 минут после удаления дроплета).
#
# Порядок именно такой: пауза уже стоит, новых партий резерв не создаёт, значит
# ожидание ограничено одним деплоем. Планировщик на паузе, поэтому анонсы дожимаем
# ручкой /api/n420/tg — и только потом снимаем БД, чтобы отметки об отправке
# уехали домой и дом не выдал те же новости повторно.
DEPLOY_WAIT_MAX="${DEPLOY_WAIT_MAX:-900}"      # потолок ожидания, сек
waited=0
while [ "$waited" -lt "$DEPLOY_WAIT_MAX" ]; do
  dp=$(api_get "$backup_base" "/api/n420/deploy-pending" 2>/dev/null) || dp=""
  n=$(jq -r '.pending // empty' <<<"$dp" 2>/dev/null)
  # Пусто — старый образ без этой ручки или CI недоступен: ждать нечего.
  [ -n "$n" ] || { log "Статус деплоя ленты недоступен — не жду."; break; }
  [ "$n" -gt 0 ] 2>/dev/null || { log "Деплой ленты завершён (ждали ${waited}с)."; break; }
  log "Деплой ленты 24n ещё идёт (прогонов: ${n}, ждём ${waited}/${DEPLOY_WAIT_MAX}с)…"
  sleep 30; waited=$((waited + 30))
done
[ "$waited" -lt "$DEPLOY_WAIT_MAX" ] || \
  log "ВНИМАНИЕ: деплой не закончился за ${DEPLOY_WAIT_MAX}с — иду дальше."

resp=$(api_post "$backup_base" "/api/n420/tg" 2>/dev/null)
if [ -n "$resp" ]; then
  log "Досылка анонсов в TG: $(jq -r '.note // "нет ответа"' <<<"$resp" 2>/dev/null)"
else
  log "Досылка анонсов в TG недоступна (старый образ на резерве) — пропускаю."
fi

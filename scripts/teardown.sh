...
#   5) только теперь удаляем дроплет.
#
# Любой сбой на шагах 1–4 ОСТАВЛЯЕТ резерв жить: лучше платить центы за час, чем
...
delete_server_fully() {
  ...
  # финальная сверка: дроплетов с нашим тегом не осталось
  local left
  left=$(find_failover_server)
  if [ -n "$left" ]; then
    log "ВНИМАНИЕ: в аккаунте всё ещё есть дроплет с тегом ${SERVER_TAG}!"
    return 1
  fi
...
# ── 5. Удаляем резерв ─────────────────────────────────────────────────────
if [ "${KEEP_SERVER:-0}" = "1" ]; then
  summary "🧪 KEEP_SERVER=1: всё проверено, но дроплет ${sid} оставлен."
  emit "torn_down=false"; exit 0
fi
delete_server_fully "$sid" || {
